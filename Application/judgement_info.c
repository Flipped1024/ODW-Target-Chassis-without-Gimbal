...
ext_radar_info_t radar_info;                             // 雷达自主决策数据
...
ext_robot_interaction_data_t robot_interaction_data;     // 学生间机器人通信数据

void Judge_Control_Init(UART_HandleTypeDef *huart)
{
    JudgeUSART = huart;
    USART_IDLE_Init(huart, judgement_receive.buf, 255);
    game_status.game_progress = PROGRESS_INIT;
}
...
void USER_UART_RxIdleCallback(UART_HandleTypeDef *huart)
{
    if (huart == JudgeUSART)
    {
        ...
        for(int i=0;i<136;i++)
            judgement_receive.bufup[i]=judgement_receive.buf[i];
        DMA_Tx2Gimbal(&huart1);
...
void DMA_Tx2Gimbal(UART_HandleTypeDef *huart)
{
    if (HAL_UART_GetState(huart) != HAL_UART_STATE_BUSY_TX && (__HAL_UART_GET_FLAG(huart, UART_FLAG_TC) == SET))
        HAL_UART_Transmit_DMA(huart,judgement_receive.bufup, sizeof(judgement_receive.bufup));
}
...
void judgement_data_decode(void)
{
    ...
    {
        memcpy(&radar_info, judgement_receive.data, sizeof(ext_radar_info_t));
    }
    break;
...
const unsigned char CRC8_TAB[256] = {
    ...
    0x6d, 0x33, 0xd1, 0x8f, 0x0c, 0x52, 0xef, 0xb1,
